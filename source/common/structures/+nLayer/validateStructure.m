...
%   RequireScalarValuesPerLayer (false) - If true, check that "er", "ur",
%       and "thk" are scalar values for each layer.
%
% Author: Matt Dvorsky
...
arguments
    ...
    options.RequireScalarValuesPerLayer(1, 1) logical = false;
end
...
if numel(er) ~= numel(ur) || numel(er) ~= numel(thk)
    error("nLayer:validateStructure:mismatchInSizeOfErUrThk", ...
        "Inputs 'er', 'ur', and 'thk' must all be cell " + ...
        "arrays with the same length (or empty).");
end

%% Check that Each Layer is Scalar
if options.RequireScalarValuesPerLayer ...
        && (any(cellfun(@numel, er) ~= 1) ...
        ||  any(cellfun(@numel, ur) ~= 1) ...
        ||  any(cellfun(@numel, thk) ~= 1))
    error("nLayer:validateStructure:nonScalarStructureValues", ...
        "The inputs 'er', 'ur', and 'thk' must be " + ...
...
for n = 1:numel(thk) - 1
    if ~all(isfinite(er{n}(:))) || ~all(isfinite(ur{n}(:))) ...
            || ~all(isfinite(thk{n}(:)))
        error("nLayer:validateStructure:nonFiniteStructureValues", ...
            "All elements of 'er', 'ur', and 'thk' except the " + ...
...
for n = 1:numel(thk)
    if ~all(real(er{n}(:)) >= 1, "all") || ~all(real(ur{n}(:)) >= 1, "all")
        error("nLayer:validateStructure:nonPassiveStructure", ...
            "The real parts of 'er' and 'ur' must be greater than 1. " + ...
            "To disable this check, set 'checkStructureValues' to false.");
    end
    if ~all(imag(er{n}(:)) <= 0, "all") || ~all(imag(ur{n}(:)) <= 0, "all")
        error("nLayer:validateStructure:nonPassiveStructure", ...
            "The imaginary parts of 'er' and 'ur' must be nonpositive. " + ...
            "To disable this check, set 'checkStructureValues' to false.");
    end
    if ~all(thk{n}(:) >= 0)
        error("nLayer:validateStructure:negativeThkValues", ...
            "All elements of 'thk' must be nonnegative. " + ...
            "To disable this check, set 'checkStructureValues' to false.");
    end
end
