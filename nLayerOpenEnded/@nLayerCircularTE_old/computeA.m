...
% member function.
A_DomMode = sum(Gamma0h .* O.fixed_AhHat(:, 1, 1), 1);
errorA_DomMode = sum(Gamma0h .* O.fixed_errorAhHat(:, 1, 1), 1);
...
A(:, :, :, fixedPointFrequency) = ...
    sum(Gamma0h(:, 1, 1, fixedPointFrequency) .* O.fixed_AhHat, 1);
