...
% member function.
A_DomMode = sum(Gamma0e .* O.fixed_AeHat(:, 1, 1), 1);
errorA_DomMode = sum(Gamma0e .* O.fixed_errorAeHat(:, 1, 1), 1);
...
A(:, :, :, fixedPointFrequency) = ...
    sum(Gamma0e(:, 1, 1, fixedPointFrequency) .* O.fixed_AeHat, 1);
