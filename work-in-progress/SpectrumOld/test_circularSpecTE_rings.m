...
x(:, 1) = 2.5 * linspace(-1, 1, 4001);
y(1, :) = 2.5 * linspace(-1, 1, 4001);
...
[xs, ys, weights] = gaussCircle(wgR, 1601, 1601);
...
kr = hypot(kx, ky);
weightFun = kr <= 0.5*max(kr, [], "all");
ExDiff = nufftn(ExSpecDiff .* weightFun, {kx, ky}, -[xSample, ySample]./(2*pi));
EyDiff = nufftn(EySpecDiff .* weightFun, {kx, ky}, -[xSample, ySample]./(2*pi));
