...
for ff = 1:numel(k0)
    ...
    valH(ff, 1) = integral(@(kr) modeFunH(kr) .* gamH(kr), 0, inf);
    valE(ff, 1) = integral(@(kr) modeFunE(kr) .* gamE(kr), 0, inf);
end
...
for ff = 1:numel(k0)
    ...
    valH(ff, 1) = integral(@(kr) modeFunH(xToKrc(kr)) .* gamH(xToKrc(kr)) .* xToKrc_weights(kr), 0, inf);
    valE(ff, 1) = integral(@(kr) modeFunE(xToKrc(kr)) .* gamE(xToKrc(kr)) .* xToKrc_weights(kr), 0, inf);
end
...
% [x, x_weights] = clenshawCurtisHalfOpen(ccOrder, ccL);
...
for ff = 1:numel(k0)
    ...
    % valH(ff, 1) = sum(gamH(xToKrc(x)) .* x_weights .* modeFunH(xToKrc(x)) .* xToKrc_weights(x));
    % valE(ff, 1) = sum(gamE(xToKrc(x)) .* x_weights .* modeFunE(xToKrc(x)) .* xToKrc_weights(x));
end
...
%% Error
err2_db = db(max(gam1 - gam2))
err3_db = db(max(gam1 - gam3))
