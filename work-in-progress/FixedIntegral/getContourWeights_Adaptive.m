function [krc, krc_weights_E, krc_weights_H] = getContourWeights_Adaptive(N, Nrho, Nphi, L, Lc, wgA, wgB)
...
% 
% MkE = sum(kr_weights .* modeFunE(kr) .* momentKernel(kr, 1:N), 1);
...
% 
% MkE = sum(kr_weights .* modeFunE(kr) .* momentKernel(kr, 1:N), 1);
% MkH = sum(kr_weights .* modeFunH(kr) .* momentKernel(kr, 1:N), 1);
% 
% [~, krc_weights_E(:, 1)] = fejer2_halfOpen(N, L, ...
%     WeightingMoments=MkE);
% 
...
for nn = 1:N
    nn
    wE = @(kr) 1 ./ sqrt(1 + kr.^2) .* modeFunE(kr) .* momentKernel(kr, nn);
    wH = @(kr) 1 .* sqrt(1 + kr.^2) .* modeFunH(kr) .* momentKernel(kr, nn);

    MkE(nn) = integral(wE, 0, inf);
    MkH(nn) = integral(wH, 0, inf);
end
